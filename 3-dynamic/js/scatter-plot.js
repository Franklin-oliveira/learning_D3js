...
function update(data) {
    ...
    let revBars = g.selectAll('circle').data(data, function (d) {
        return d.month;    // the elements in the array are matched according to the month
    })
    ...
    revBars.enter()
        .append('circle')
            ...
            .attr('fill', '#d9b38c')
            .attr('cy', y(0))
        .merge(revBars)  // UPDATE old elements present in new data
        .transition(t)
            .attr('cy', function (d) {
                return y(d[value]);
            })
